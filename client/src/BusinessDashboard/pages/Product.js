...
const ProductTable = () => {
  ...
  const handleCloseDialog = () => {
    setOpenDialog(false);
    setCurrentProduct({
      id: null, businessId: '', category: '', name: '', price: '', discount: '', stock: '', details: '', images: []
    });
    setNewImages([]);
    setDeletedImages([]);
  };
  ...
  const handleImageUpload = (e) => {
    ...
    const maxNew = 5 - (currentProduct.images.length - deletedImages.length) - newImages.length;
  ...
  const handleSubmit = async () => {
    const { businessId, category, name, price, stock, details, discount } = currentProduct;
  ...
  const allImages = [...currentProduct.images.map(url => ({ url, isNew: false })), ...newImages];

  return (
    <Box p={3}>
      <Box display="flex" justifyContent="space-between" mb={2}>
        <Typography variant="h5" fontWeight="bold" color="primary">Product Catalog</Typography>
        <Button variant="contained" startIcon={<Add />} onClick={handleOpenAddDialog}>Add Product</Button>
      </Box>
              ...
              <TableBody>
                {products.length === 0 ? (
                  ...
                  <StyledTableRow key={p._id}>
                    ...
                    <TableCell align="right">{p.discount ? `${p.discount}%` : 'N/A'}</TableCell>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        <DialogTitle>
          ...
          <IconButton onClick={handleCloseDialog} sx={{ position: 'absolute', top: 8, right: 8 }}><Close /></IconButton>
        </DialogTitle>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select name="businessId" value={currentProduct.businessId} onChange={handleInputChange} label="Business">
                  {businesses.map(b => <MenuItem key={b._id} value={b._id}>{b.businessName}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12}><TextField fullWidth name="name" label="Name*" value={currentProduct.name} onChange={handleInputChange} /></Grid>
            <Grid item xs={12}><TextField fullWidth name="category" label="Category" value={currentProduct.category} onChange={handleInputChange} /></Grid>
            <Grid item xs={6}><TextField fullWidth name="price" type="number" label="Price*" value={currentProduct.price} onChange={handleInputChange} /></Grid>
            <Grid item xs={6}><TextField fullWidth name="stock" type="number" label="Stock*" value={currentProduct.stock} onChange={handleInputChange} /></Grid>
            <Grid item xs={6}><TextField fullWidth name="discount" type="number" label="Discount" value={currentProduct.discount} onChange={handleInputChange} /></Grid>
            <Grid item xs={12}><TextField fullWidth multiline rows={3} name="details" label="Details" value={currentProduct.details} onChange={handleInputChange} /></Grid>
            <Grid item xs={12}>
              <Button
                fullWidth
                component="label"
              ...
              <Box mt={1} display="flex" gap={1} flexWrap="wrap">
                {allImages.map((img, idx) => (
                  <Box key={img.url || idx} sx={{ position: 'relative' }}>
                    <img src={img.url} width={80} height={80} style={{ objectFit: 'cover' }} />
                    <IconButton
